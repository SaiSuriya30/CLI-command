...
def save_to_json(command, output, ceph_version):
    ...
    if match:
        ...
        combined_string = f"{command}{json.dumps(output, sort_keys=True)}"
        output_hash = hashlib.sha256(combined_string.encode('utf-8')).hexdigest()
        ...
        if not duplicate_found:
            ...
            print(f"Duplicate output found for command: {command} - skipping.")
