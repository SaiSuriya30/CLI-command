...
def save_to_json(command, output, ceph_version):
    # Check if the folder "Suriya" exists; if not, create it
    ...
    # Extract the subcommand from the command string
    ...
    if match:
        ...
        # Check if the file exists; if yes, load existing data
        if os.path.exists(file_name):
            ...
            data = {"ceph_version": ceph_version, "outputs": []}  # Initialize data if file doesn't exist

        if "outputs" not in data:
            data["outputs"] = []

        # Append the new command and output to the data
        data["outputs"].append({"command": command, "output": output})
